...
$er = isset($_POST['btnSInscrire']) ? hrl_traitement_inscription() : array(); 

/*------------------------- Etape 2 --------------------------------------------
- génération du code HTML de la page
------------------------------------------------------------------------------*/

hr_aff_debut('Cuiteur | Inscription', '../styles/cuiteur.css');

hr_aff_entete_empty('Inscription');
hr_aff_infos(false);

hrl_aff_formulaire($er);
 ...
 * @global  array   $_POST
 */
function hrl_aff_formulaire(array $err): void {
    ...
    hr_aff_ligne_input( 'Votre pseudo :', array('type' => 'text', 'name' => 'pseudo', 'value' => $values['pseudo'], 
                        'placeholder' => 'Minimum 4 caractères alphanumériques', 'required' => null));
    hr_aff_ligne_input('Votre mot de passe :', array('type' => 'password', 'name' => 'passe1', 'value' => '', 'required' => null));
    hr_aff_ligne_input('Répétez le mot de passe :', array('type' => 'password', 'name' => 'passe2', 'value' => '', 'required' => null));
    hr_aff_ligne_input('Nom et prénom :', array('type' => 'text', 'name' => 'nomprenom', 'value' => $values['nomprenom'], 'required' => null));
    hr_aff_ligne_input('Votre adresse email :', array('type' => 'email', 'name' => 'email', 'value' => $values['email'], 'required' => null));
    hr_aff_ligne_input('Votre date de naissance :', array('type' => 'date', 'name' => 'naissance', 'value' => $values['naissance'], 'required' => null));
 ...
 * et donc entraînent l'appel de la fonction hr_session_exit() sauf :
 ...
 */
function hrl_traitement_inscription(): array {
    
    if( !hr_parametres_controle('post', array('pseudo', 'email', 'nomprenom', 'naissance', 
    ...
    if (count($erreurs) == 0) {
        ...
        $pseudo = hr_bd_proteger_entree($bd, $_POST['pseudo']);
    ...
    $email = hr_bd_proteger_entree($bd, $_POST['email']);
    
    $passe1 = password_hash($_POST['passe1'], PASSWORD_DEFAULT);
    $passe1 = hr_bd_proteger_entree($bd, $passe1);
